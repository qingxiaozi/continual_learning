...
1. 对比实验设计
Base_Uniform：带宽均匀分配，数据按固定比例上传，缓存采用FIFO策略。<br>
Abl_BW_Opt：带宽最小化最大时延分配，数据按固定比例上传，缓存采用FIFO策略。<br>
Abl_UP_Greedy：带宽贪婪分配，数据由DRL决策上传，缓存采用FIFO策略。<br>
Abl_UP_DRL：带宽均匀分配，数据由DRL决策上传，缓存采用FIFO策略。<br>
Abl_NoReplay：带宽均匀分配，数据按固定比例上传，仅使用新数据训练。<br>
Combo_Comm：带宽最小化最大时延分配，数据由DRL决策上传，缓存采用FIFO策略。<br>
Combo_Learn：带宽均匀分配，数据由DRL决策上传，缓存采用MAB策略。<br>
Ours_Full：带宽最小化最大时延分配，数据由DRL决策上传，缓存采用MAB策略。<br>
